...
def update_objects(data_list):
    ...
    for item in data_list:
        ...
        if label == 'robot_body':
            robot_body = coords
        elif label == 'top-left_knob':
            top_left_knob = coords
        elif label == 'top-right_knob':
            top_right_knob = coords
        elif label == 'bottom_knob':
            bottom_knob = coords
        elif label == 'bottom_space':
            bottom_space = coords
        elif label == 'top-left_space':
            top_left_space = coords
        elif label == 'middle-right_knob':
            middle_right_knob = coords
        elif label == 'middle-left_knob':
            middle_left_knob = coords
        elif label == 'blue_cube':
            blue_cube = coords
        elif label == 'robot_gripper':
            robot_gripper = coords
        elif label == 'cabinet_all':
            cabinet_all = coords

    return {
        "robot_body": robot_body,
        "top_left_knob": top_left_knob,
        "top_right_knob": top_right_knob,
        "bottom_knob": bottom_knob,
        "bottom_space": bottom_space,
        "top_left_space": top_left_space,
        "middle_right_knob": middle_right_knob,
        "middle_left_knob": middle_left_knob,
        "blue_cube": blue_cube,
        "robot_gripper": robot_gripper,
        "cabinet_all": cabinet_all
    }
    ...
    return robot_body, top_left_knob, top_right_knob, bottom_space, blue_cube, robot_gripper
#----------------
i=0
     ...
     while not rospy.is_shutdown():
        try:
            ...
            if converted_list is not None:
                ...
                print(results)
                # 检查 'blue_cube' 和其他项目是否存在
                if 'top_right_knob' in results and len(results['top_right_knob']) >= 2 :
                    
                    # 计算3D点

                    X5, Y5, D5 = pixel_to_3d_point(results["top_right_knob"][0], results["top_right_knob"][1])
                    # 创建 Marker 数据
                    marker_data = {

                        5: (X5, Y5, D5)   # Marker ID: 3
                    ...
                    print("Data missing, waiting for next update...")

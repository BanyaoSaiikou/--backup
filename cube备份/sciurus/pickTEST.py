...
def callback3(data):
    ...
    for marker in data.markers:
        ...
        if i==1:
            bottom_spaceX=[marker.pose.position.x+0.14979217, marker.pose.position.y+0.02889325, marker.pose.position.z+0.57816425]
        if i==2:
            top_left_knobX=[marker.pose.position.x+0.14979217, marker.pose.position.y+0.02889325, marker.pose.position.z+0.57816425]
        if i==3:
            top_right_knobX=[marker.pose.position.x+0.14979217, marker.pose.position.y+0.02889325, marker.pose.position.z+0.57816425]
        i+=1
...
def grasp_object(gripper, gripper_goal):
    gripper_goal.command.position = -0.1#cube:-0.3 ,knob:-0.2 
    gripper.send_goal(gripper_goal)
    gripper.wait_for_result(rospy.Duration(1.0))
...
def map_object(object):
    ...
    # Initialize robot and gripper
        ...
        if object == 'blue_cube':
            ...
            object =bottom_space
        elif object == 'top_left_knob':
            object =top_left_knob
        elif object == 'top_right_knob':
